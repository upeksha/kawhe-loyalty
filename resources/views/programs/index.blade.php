        ...
        <x-ui.page-header title="{{ __('Loyalty Cards') }}">
            <x-slot name="actions">
                @if($usageStats['can_create_program'] ?? false)
                    <x-ui.button href="{{ route('merchant.stores.programs.create', $store) }}" variant="primary" size="sm">
                        Add Loyalty Card
                    </x-ui.button>
                @endif
            </x-slot>
    ...
    <div class="space-y-6">
        @if($activePrograms->isEmpty() && $archivedPrograms->isEmpty())
            <x-ui.empty-state
                ...
                description="{{ ($usageStats['can_create_program'] ?? false) ? 'Create your first loyalty card to get a join link and QR code for customers.' : 'Your plan limit for loyalty cards on this store has been reached. Upgrade to add more cards.' }}"
            >
                @if($usageStats['can_create_program'] ?? false)
                    <x-ui.button href="{{ route('merchant.stores.programs.create', $store) }}" variant="primary">
                        Add Loyalty Card
                    </x-ui.button>
                @else
        ...
        @if($activePrograms->isNotEmpty())
            <div>
                <div class="mb-3 flex items-center justify-between gap-3">
                    <h2 class="text-sm font-semibold uppercase tracking-wide text-stone-500">Active Cards</h2>
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    <div class="flex items-center gap-2">
                                        <h3 class="font-semibold text-stone-900">{{ $program->name }}</h3>
                                    ...
                                    <p class="text-sm text-stone-600 mt-2">{{ $program->reward_target }} stamps for {{ $program->reward_title }}</p>
                                    <p class="text-sm text-stone-500 mt-1">{{ $program->loyalty_accounts_count }} customer cards joined</p>
                                </div>
                            </div>
                            <div class="mt-4 flex items-center gap-3">
                                <a href="{{ route('merchant.stores.programs.qr', [$store, $program]) }}" class="text-brand-600 hover:text-brand-700 font-medium text-sm">QR Code</a>
                                <a href="{{ route('merchant.stores.programs.edit', [$store, $program]) }}" class="text-brand-600 hover:text-brand-700 font-medium text-sm">Edit</a>
                                @if(! $program->is_default)
                                    <form method="POST" action="{{ route('merchant.stores.programs.destroy', [$store, $program]) }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-brand-600 hover:text-brand-700 font-medium text-sm">Archive</button>
                                    </form>
                                @endif
                            </div>
                        </x-ui.card>
