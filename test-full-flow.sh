...
echo "  php artisan tinker --execute=\"\$a = \App\Models\LoyaltyAccount::latest()->first(); if (\$a) { \$s = \\App\\Services\\Wallet\\Apple\\AppleWalletSerial::fromAccount(\$a); \$c = \\App\\Models\\AppleWalletRegistration::where('serial_number', \$s)->where('active', true)->count(); echo 'Registrations: ' . \$c . PHP_EOL; }\""
echo ""
echo "Test push:"
